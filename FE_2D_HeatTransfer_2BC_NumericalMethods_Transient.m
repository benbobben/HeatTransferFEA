...
% NOTE : Might not run out of the box - Using distmesh package
...
% T = temperature
% Q = heat energy generated per unit area per unit time W atts = Joule=sec
...
addpath("/Users/benjackson/Desktop/School/Grad -Spring/Neumerical Methods/Final/Mesh /distmesh-master/") %Specify PKG location
...
time = 0:.5:10;
temperature_C = zeros(length(nodeLocationTable),length(time));
for t_i = 1:length(time)
    ...
    temperature_C(:,t_i) = inv(globalMatrix) * rhs; % Temperature solve for deg C 
end
...
subplot(1,3,1);
trisurf(transformationTable,nodeLocationTable(:,1), nodeLocationTable(:,2), temperature_C(:,1));
title('time = 0s');
xlabel('X (m)');
ylabel('Y (m)');
% colorbar;
clim([-23 30])
minTemp = min(min(temperature_C))
maxTemp = max(max(temperature_C))
xlim([0 x_max])
ylim([0 y_max])
zlim([minTemp maxTemp])
valHold  = maxTemp
fondSize = 40
...
subplot(1,3,2);
% figure
trisurf(transformationTable,nodeLocationTable(:,1), nodeLocationTable(:,2), temperature_C(:,2));
title('time = .5s');
xlabel('X (m)');
ylabel('Y (m)');
% colorbar;
...
subplot(1,3,3);
% figure
trisurf(transformationTable,nodeLocationTable(:,1), nodeLocationTable(:,2), temperature_C(:,end));
title('time = End');
xlabel('X (m)');
ylabel('Y (m)');
colorbar;
